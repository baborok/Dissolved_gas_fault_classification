...
layout = [[sg.Graph((640, 480), (0, 0), (640, 480), key='Graph1'), [sg.Text('Индикатор состояния трансформатора', size=(30,1))],[sg.Text('Индекс'), LEDIndicator('_cpu_')]]]
...
while True:
    ...
    if event == sg.WINDOW_CLOSED:
        ...
        animate(100,xs,ys)
    SetLED(window, '_cpu_', 'green' if cock() >= 56 else 'red')
...
#plt.show()
    ...
    #if x >= 56:
        #ransformer_index = f'Индекс состония трансформатора 1 с процентом оставшегося ресурса {x}'
   # elif 29<=x<=55:
    #    transformer_index = f'Индекс состония трансформатора 2 с процентом оставшегося ресурса {x}'
    #elif 17<=x<=28:
   #     transformer_index = f'Индекс состония трансформатора 3 с процентом оставшегося ресурса {x}'
